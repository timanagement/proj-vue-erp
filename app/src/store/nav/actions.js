...
export default {
   ...
   * 显示隐藏全部通知信息
   * */
  toggle_show_all_notification ({dispatch, commit}) {
    return dispatch('hide_notification').then(() => {
      new Promise((resolve, reject) => {
   ...
   * 显示隐藏用户设置
   * */
  toggle_show_usersetting ({commit}) {
    return new Promise((resolve, reject) => {
      commit(types.TOGGLE_SHOW_USERSETTING);
      resolve()
    });
  },
  /**
   * 隐藏用户设置
   * */
  hide_usersetting ({commit}) {
    return new Promise((resolve, reject) => {
      commit(types.HIDE_USERSETTING);
      resolve()
    });
  },
};
